...
def profile_point(key):
        ...
        print("times[{}]: {}s".format(key, times[key]))
...
import warnings
warnings.filterwarnings("ignore", message="Matplotlib is currently using pdf, which is a non-GUI backend, so cannot show the figure.")

profile_point("imports")

###############################################################################
# Set parameters
dataset_location=cur_dir + "/datasets/mne/"
os.makedirs(dataset_location, exist_ok=True)
data_path = somato.data_path(dataset_location)
out_dir = cur_dir + "/outputs/"
...
epochs.resample(600., npad='auto')  # resample to reduce computation time

profile_point("data_sample")
...
plt.savefig(out_dir + "epochs-avg.pdf")
profile_point("q1")

###############################################################################
# Now let's take a look at the spatial distributions of the PSD.
epochs.plot_psd_topomap(ch_type='grad', normalize=True)
plt.savefig(out_dir + "spat-dist-psd.pdf")
profile_point("q2")
...
# plt.show()
plt.savefig(out_dir + "multipaper-psd.pdf")
profile_point("q3")
...
# plt.show()
plt.savefig(out_dir + "welch-psd.pdf")
profile_point("q5")
...
plt.savefig(out_dir + "power-avg.pdf")
profile_point("q8")

power.plot([82], baseline=(-0.5, 0), mode='logratio', title=power.ch_names[82])
plt.savefig(out_dir + "power-" + power.ch_names[82] + ".pdf")
profile_point("q9")
...
plt.savefig(out_dir + "power-alpha-beta.pdf")
profile_point("q10")
...
plt.savefig(out_dir + "power-joint.pdf")
profile_point("q11")
...
for tt in times.keys():
        if "q" in tt:
                query_sum += times[tt]
times["queries"] = query_sum

print(times)
print("profile_times:::" + str(json.dumps(times)))
total = 0
