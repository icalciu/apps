...
# Check if we specified startup script
if [ ! -z "${VOLTDB_CLIENTPORT}" ]; then
    PORT_STR="--port=${VOLTDB_CLIENTPORT}"
fi
...
# shutdown voltdb server
function servershutdown() {
    jars-ifneeded
    sqlcmd ${PORT_STR} --query="exec @Shutdown"
}

function help() {
    echo "Usage: ./run.sh {clean|jars|server|init|client|servershutdown}"
}
